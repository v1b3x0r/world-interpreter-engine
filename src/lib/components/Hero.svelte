...
>
	...
	{#if showPunch}
		...
		>
			<div class="wl-card flex-1 p-4">
				<div class="mb-2 text-xs text-[var(--wl-text-muted)]">{m.hero_context_home()}</div>
				<div class="text-sm text-[var(--wl-text)]">{punchHome.text}</div>
			</div>
			<div class="wl-card flex-1 p-4">
				<div class="mb-2 text-xs text-[var(--wl-text-muted)]">{m.hero_context_space()}</div>
	...
	<div class="mt-2 flex gap-12 text-[10px] text-[var(--wl-text-muted)]">
		<span>{m.hero_hint_explore()}</span>
		<span>{m.hero_hint_editor()}</span>
	</div>
